...
blogsRoutes.get('/', async (req, res) => {
  res.send(await blogsServices.findAllBlogs(req?.query?.paseSize?.toString() || '10', req?.query?.pageNumber?.toString() || '1', req?.query?.searchNameTerm?.toString() || null, req?.query?.sortBy?.toString() || 'createdAt', req?.query?.sortDirection?.toString() || 'desc'))
});
...
blogsRoutes.get('/:id/posts', async (req, res) => {
  ...
  res.send(await blogsServices.findAllPostsByBlogId(req?.query?.paseSize?.toString() || '10', req?.query?.pageNumber?.toString() || '1', req?.query?.sortBy?.toString() || 'createdAt', req?.query?.sortDirection?.toString() || 'desc', req?.params?.id?.toString()))
});
